...
RES=$(curl -s -X POST "$BASE/auth/login" -H "Content-Type: application/json" -d '{"employeeId":"ADV001","password":"agent123"}')
...
if echo "$RES" | grep -q '"success":true' && echo "$RES" | grep -q '"status":"pending"'; then pass "Registration submitted"; else fail "Registration FAILED: $RES"; fi
...
if [ -n "$REG_ID" ] && [ "$REG_ID" != "" ]; then
  ...
  if echo "$RES" | grep -q '"success":true' && echo "$RES" | grep -q 'userId'; then pass "Registration approved"; else fail "Approval FAILED: $RES"; fi
else
...
HTTP=$(python3 - <<'PY' | curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/auth/login" -H "Content-Type: application/json" --data-binary @-
import json
print(json.dumps({"employeeId": "A" * 100000, "password": "test"}))
PY
)
