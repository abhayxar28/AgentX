...
export default function AgentDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    api.get(`/admin/agents/${id}`).then(res => setDetail(res.data.data)).catch(() => navigate('/admin/agents')).finally(() => setLoading(false));
  }, [id, navigate]);
    ...
    <div className="space-y-6">
      <section className="page-heading">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={() => navigate('/admin/agents')} className="btn-secondary">Back to Overview</button>
        </div>
      </section>
