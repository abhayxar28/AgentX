...
export async function registerAdmin(req: Request, res: Response): Promise<void> {
  try {
    const { name, employeeId, branchId, email, password } = req.body;
    ...
    await AuditLog.create({
      ...
      diff: { before: {}, after: { employeeId: newUser.employeeId, name: newUser.name, role: 'admin' } },
    });

    await sendNotification({
      to: { email, name },
      type: 'admin_registration_success',
      data: { employeeId: newUser.employeeId },
    });
    ...
    sendError(res, 'INTERNAL_ERROR', 'Admin registration failed', 500);
  }
}
...
export async function approveRegistration(req: Request, res: Response): Promise<void> {
  try {
    ...
    const existingUser = await User.findOne({ employeeId: registration.employeeId });
    if (existingUser) {
    ...
    // Keep side effects best-effort so approval itself does not fail.
    try {
      await AuditLog.create({
        action: 'user_created',
      ...
      logger.error('Failed to write approval audit log', error);
    }
    ...
    logger.error('approveRegistration failed', e);
